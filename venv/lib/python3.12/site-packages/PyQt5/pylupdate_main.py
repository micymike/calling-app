...
def main():
    ...
    while i < argc:
        ...
        if arg == "-tr-function":
            if i >= argc:
                sys.stderr.write("pylupdate5 error: missing -tr-function name\n")
                sys.exit(2)
        ...
        if arg == "-translate-function":
            if i >= argc:
                sys.stderr.write("pylupdate5 error: missing -translate-function name\n")
                sys.exit(2)
        ...
        if standardSyntax:
            ...
            for key, value in proFileTagMap(fullText).items():
                for t in value.split(" "):
                    if key == "SOURCES":
                        fetchtr_py(
                            _encoded_path(QDir.current().absoluteFilePath(t)),
                            fetchedTor,
                            defaultContext,
                            True,
                            codecForSource,
